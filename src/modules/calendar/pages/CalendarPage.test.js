...
jest.mock("../network/calendar.api.js", () => ({
  getCityWeatherInDate: () => ({ weather: {} }),
}));
...
test("should load all days in calendar correctly", () => {
  render(<CalendarPage />);
...
test("should open add dialog when day is clicked", async () => {
  ...
  const seeReminderDialog = await screen.findByText("See reminder details");
  expect(seeReminderDialog).toBeVisible();

  const editReminderButton = screen.getByRole("button", {
    name: "Edit Reminder",
  });
